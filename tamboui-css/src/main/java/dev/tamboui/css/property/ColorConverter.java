...
/**
 * Converts CSS color values to TamboUI Color objects.
 * <p>
 * Supported formats:
 * <ul>
...
public final class ColorConverter implements PropertyConverter<Color> {
    ...
    public Optional<Color> convert(String value, Map<String, String> variables) {
        ...
        // Hex color
        Matcher hexMatcher = HEX_PATTERN.matcher(resolved);
        if (hexMatcher.matches()) {
            return Optional.of(parseHex(hexMatcher.group(1)));
        }

        // RGB function
        Matcher rgbMatcher = RGB_PATTERN.matcher(resolved);
        ...
        // Indexed color
        Matcher indexedMatcher = INDEXED_PATTERN.matcher(resolved);
