...
* Terminal I/O errors are wrapped in `TerminalIOException` (`dev.tamboui.errors.TerminalIOException`), to avoid having to catch `IOException` which often are non-recoverable.
